...
function fakeSummary(title: string, tz: 'HKT' | 'ET', lang: 'en' | 'zh-HK'): TaskSummary {
...
function seedTasks(): Task[] {
  ...
  const a = new Date(now); a.setHours(7,30,0,0);
  const b = new Date(now); b.setHours(8,0,0,0);
  const c = new Date(now); c.setHours(21,0,0,0);
  const tasks: Task[] = [
    {
      id: 'hk-morning',
...
export function useTasks(): UseTasksApi {
  ...
  useEffect(() => { if (tasks.length) persist(tasks); }, [tasks]);

  const select = useCallback((id?: string) => setSelectedId(id), []);

  const create = useCallback((partial: Partial<Task> & { title: string }): Task => {
    const task: Task = {
  ...
  const run = useCallback((task: Task) => {
    ...
    return { ...task, status: 'done-today', lastRunAt: nowIso, lastSummary: summary } as Task;
  }, []);
  ...
  return { tasks, selected, select, create, update, remove, runAll, runOne, regenerate };
}
